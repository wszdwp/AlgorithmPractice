...
public class DamageCardGame {
    ...
    //                    if (helper(totalD - damages[i], totalM - costs[i], costs, damages, i + 1))
    // return true;
    //                }
    //                return false;
    //            }
    ...
    private boolean helper(
            int totalD, int totalM, int[] costs, int[] damages, int start, List<Integer> path) {
        if (totalD == 0 && totalM >= 0) {
            ...
            if (start == damages.length || totalD < 0 || totalM < 0) {
                ...
                for (int i = start; i < damages.length; i++) {
                    path.add(i);
                    if (helper(totalD - damages[i], totalM - costs[i], costs, damages, i + 1, path))
                        return true;
