...
public class AirFlightRouteSchedule {
    /**
     * A flight has maximum distance needs to be assigned one forward and one return route, A route is
     * {routeId, distanceOfRoute} structure Given a list of forward routes and a list of return
     * routes, assign one forward and one return to given flight,achieve optimum schedule the result
     * returned is a list of {{forward_routeId, return_routeId}}
     *
     * <p>A optimal route schedule has two conditions: 1. sum of the forward and backward distance
     * cannot exceeds limit of the flight distance 2. no other other routes are better than the
     * assigned routes pair
     *
     * <p>one route can only be assigned one time
     *
     * <p>eg1: maxDistance: 20 forwardRoutes: {{1, 4}, {2, 8}, {3, 14}} returnRoutes: {{1, 2}, {2, 9},
     * {3, 14}} return {{1, 3}}
     *
     * <p>eg1: maxDistance: 20 forwardRoutes: {{1, 1}, {2, 7}, {3, 2}} returnRoutes: {{1, 3}, {2, 9},
     * {3, 5}} return {{2, 1}, {1, 2}} or {{1, 1}, {2, 2}}
     *
     * @param maxDistance
     * @param forwardRoutes
     * @param returnRoutes
     * @return
     */
    public List<List<Integer>> scheduleRoute(
            int maxDistance, List<List<Integer>> forwardRoutes, List<List<Integer>> returnRoutes) {
    ...
    public static void main(String[] args) {
        ...
        forwardRoutes.add(Arrays.asList(1, 3));
        forwardRoutes.add(Arrays.asList(2, 6));
        forwardRoutes.add(Arrays.asList(3, 14));

        List<List<Integer>> returnRoutes = new ArrayList<>();
        returnRoutes.add(Arrays.asList(1, 2));
        returnRoutes.add(Arrays.asList(2, 9));
        returnRoutes.add(Arrays.asList(3, 14));
