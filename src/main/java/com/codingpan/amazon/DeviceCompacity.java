...
public class DeviceCompacity {
    /**
     * 第二题就是 DeviceCapacity。
     * Input类似是这样
     * front = [[1,3000],[2,5000],[3,7000],[4,10000]]；
     * back =[[1,2000],[2,3000],[3,4000],[4,5000]]，
     * Capacity = 10000,
     * 要求就是所有front & back 的combination中最大的且<= Capacity的id number。
     * 像这个Input就要output出 [[2,4],[3,2]]。
     * 是的要考虑有可能有相同最多的且Valid的Capacity的组合。
     * 还有另外一个情景就是那个Amazon Air的Optimal 距离。也是给两个list，然后return最optimal的组合。
     * 个人用的是暴力解，由于因为一定要从两个list中各取一个，lz觉得暴力解最直接，
     * 也不会让code看起来too complicated。如果有大神有更好的解法，欢迎讨论！
     */
    // O(N^2)
