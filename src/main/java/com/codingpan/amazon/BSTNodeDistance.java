...
public class BSTNodeDistance {
    ...
    /**
     * 给定一个integer数组（无序）和2个node值，求构建bst(不可调整树）后，两个node的距离。 Given a list of numbers, construct a BST
     * from it and find the distance between two nodes.
     */
    public int bstDistance(int[] values, int node1, int node2) {
        ...
        //        TreeNode lca = lowestCommonAncestorBST(root, new TreeNode(node1), new
        // TreeNode(node2));
